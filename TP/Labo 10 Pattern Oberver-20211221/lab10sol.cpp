...
class SumRectangle: 
    public IntRectangle,
    public Observer {
 vector<shared_ptr<IntRectangle>> toSum;
 public:
  SumRectangle(Point center, int w, int h,const vector<shared_ptr<IntRectangle>> &toSum,int textSize=10)
      : IntRectangle{center, w, h, 0,textSize}, toSum{toSum} {
        for (auto iRect:toSum) iRect->registerObserver(this);
        subjectChanged();
      }
  void subjectChanged() override {
    int sum=0;
    for (auto iRect:toSum) sum+=*iRect;
    setInteger(sum);
  }
};
...
class MultRectangle: 
    public IntRectangle,
    public Observer {
 vector<shared_ptr<IntRectangle>> toMult;
 public:
  MultRectangle(Point center, int w, int h,const vector<shared_ptr<IntRectangle>> &toMult,int textSize=10)
      : IntRectangle{center, w, h, 0,textSize}, toMult{toMult} {
        for (auto iRect:toMult) iRect->registerObserver(this);
        subjectChanged();
      }
  void subjectChanged() override {
    int product=1;
    for (auto iRect:toMult) product*=*iRect;
    setInteger(product);
  }
};
...
Canvas::Canvas() {
  ...
  drawables.push_back(make_shared<SumRectangle>(Point{150,250},50,50,
    vector<shared_ptr<IntRectangle>>
      {dynamic_pointer_cast<IntRectangle>(drawables[0]),
      dynamic_pointer_cast<IntRectangle>(drawables[3])}, textSize
    ));
  drawables.push_back(make_shared<SumRectangle>(Point{350,250},50,50,
    vector<shared_ptr<IntRectangle>>
      {dynamic_pointer_cast<IntRectangle>(drawables[3]),
        dynamic_pointer_cast<IntRectangle>(drawables[6])}, textSize
    ));

  //This is the product
  drawables.push_back(make_shared<MultRectangle>(Point{250,150},100,50,
    vector<shared_ptr<IntRectangle>>
      {dynamic_pointer_cast<IntRectangle>(drawables[9]),
        dynamic_pointer_cast<IntRectangle>(drawables[10])}, textSize
    ));
